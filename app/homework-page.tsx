...
const district = practice.districtResources;
...
export default function HomeworkPage() {
  ...
  return <main className={`content-page homework-page${includeChecks ? " include-checks" : ""}`}>
    ...
    <section id="surrey-resources" className="homework-resources">
      <h2>Surrey Schools resources</h2><p>{district.intro}</p>
      <p><a href={district.directory} target="_blank" rel="noreferrer">Browse Surrey’s student resources ↗</a> · <a href={district.parents} target="_blank" rel="noreferrer">Parent resource directory ↗</a></p>
      <p><strong>Getting access:</strong> {district.access}</p>
      <p>District listings checked {district.checkedOn}. Resource titles below open Surrey’s own information pages. The short practice ideas are suggestions from this class, not district assignments.</p>
      <div>{district.items.map(item => <article key={item.title}>
        <small>{item.subject}</small><h3><a href={item.href} target="_blank" rel="noreferrer">{item.title} ↗</a></h3>
        <p><strong>{item.status}</strong></p><p>{item.description}</p>
